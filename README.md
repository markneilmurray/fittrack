...
- **Exercise library** — 75 exercises across chest, back, shoulders, arms, legs, core and cardio, each with a real photo, step-by-step instructions and target muscles (sourced from the public-domain [free-exercise-db](https://github.com/yuhonas/free-exercise-db) project), plus a built-in HIIT option under Cardio. Star any exercise to favourite it. Can't find what you did? Tap **"Add a custom exercise"** at the bottom of the library to add your own (name, type, muscle group, optional notes) — it behaves exactly like a built-in one everywhere, just without a stock photo.
- **Session builder** — start from a Push/Pull/Legs/Upper/Lower/Full Body/Core/Cardio/HIIT template or build a custom session, with sensible default sets & reps you can adjust. Edit a template's exercises once and it's remembered as the new default for next time — a "reset to default" link is always there if you want the original back. Built a one-off custom session that's actually worth repeating? **"Save as a template"** turns it into a new entry on the Train tab, right alongside the built-in ones, editable/deletable the same way.
- **Workout logging** — log weight and reps per set, with the weight/duration you did **last time** shown as a reminder so you know what to beat. Cardio exercises have an editable duration/distance while you're doing them, plus an optional **calories burned** field — anything logged there is added back onto that day's food allowance (Food page and the home screen's Calories tile both reflect it, e.g. a 2000 kcal goal plus a 250 kcal burn shows as a 2250 kcal allowance for the day).
...
- **Food log** — log meals with calories and macros against a daily goal. Type what you ate and tap **"Look up calories for this"** for an AI estimate (handles casual descriptions like "a bourbon biscuit" or "cup of tea with milk"), or tap **"Estimate from a photo"** to do the same from a picture instead — the photo is sent once for the estimate and never saved anywhere. See **AI meal estimate** below. Past lookups are remembered on-device: typing something you've searched before shows it as a suggestion that fills in instantly with no AI call, and starring a search adds it to a **Quick add** row on the Food page for one-tap logging. Ate something twice? Each entry has **−/+** to adjust how many, scaling its calories/macros and the day's totals accordingly, instead of re-adding it from scratch.
...
**"Hit today's free AI usage limit" errors:** the free tier is genuinely small — a `429 RESOURCE_EXHAUSTED` with `GenerateRequestsPerDayPerProjectPerModel-FreeTier` in the details means this Firebase project has used its **20 requests/day** free allowance for that model. If this project is shared with other apps (as `maths-minutes` is, across FitTrack/profiles/english/reading), all of them draw from the same 20/day pool — heavy use of one can lock out the others until the daily reset. Check **Firebase console → AI Logic → AI monitoring → Traces** to see exactly which requests are failing and when. Fix: wait for the daily reset, split heavy-usage apps onto separate Firebase projects, or upgrade the project to the Blaze plan (pay-as-you-go removes the free-tier ceiling; typical personal use still costs fractions of a cent).
...
Those stats — plus the actual list of everything logged that week (food names and calories, exercises done) — are then handed to Gemini (same AI Logic connection as the food estimates) to turn into 3-5 short, specific suggestions plus a one-line headline. The prompt specifically asks it to name real foods from your log (not just talk in totals — e.g. calling out a specific high-calorie takeaway as a target to cut back on, or a good protein choice worth repeating) and to give a clear steer on strength vs. cardio balance based on your actual session counts vs. goals. Nothing here is saved anywhere except the summary text itself (in your own profile data) — it's cached locally so it's not silently regenerated (and re-billed) every time you open the app; tap **Refresh** for a new one anytime.
